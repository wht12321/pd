...
function myworks(){
    ...
    xhr.onreadystatechange=function(){
        if(xhr.readyState==4&&xhr.status==200){
            ...
            //console.dir(xhr.responseText);//已经获得了uhead和自己的作品！！！！！！！
            //alert(www[0].pid)
            for(var elem of www){
                ggo.innerHTML+=`<div><div><a href="http://localhost:8080/workshow.html"><img src="${elem.phead}" data-num="${elem.pid}"></a></div><button>删除</button></div>`
            }
        }
    }
    xhr.open("post","/user/mws",true);
...
//删除图片ggo
    ggo.onclick=function(e){
        ...
        if(a.nodeName==="BUTTON"){
            ...
            xhr.open("get","/user/dp?pid="+duiyinimg.getAttribute("data-num"),true);
            xhr.send();            
        }
    //点击图片跳转详情页面
    if(a.nodeName==="IMG"){
